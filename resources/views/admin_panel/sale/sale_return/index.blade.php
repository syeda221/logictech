...
@section('content')
    <style>
        /* ==========================================================================
           Enterprise ERP Design System - Sale Returns Management
           ========================================================================== */
        ...
        <div class="main-content-inner">
            <div class="container-fluid py-4">
                ...
                <div class="erp-page-header d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3">
                    <div>
                        <h1 class="erp-title">
                            <i class="fas fa-undo-alt text-primary"></i> Sale Returns
                        </h1>
                        <p class="erp-subtitle">Manage customer returns, product replacements, ledger credits, and cash refunds</p>
                    </div>
                ...
                @php
                    ...
                    $totalDueSettled = (float) $returns->sum('due_adjusted');
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-xl-3 col-sm-6">
                        <div class="erp-kpi-card">
                            <div>
                                ...
                                <div class="erp-kpi-value text-dark font-monospace">Rs {{ number_format($totalReturnValue, 2) }}</div>
                            </div>
                    ...
                    <div class="col-xl-3 col-sm-6">
                        <div class="erp-kpi-card">
                            <div>
                                <div class="erp-kpi-label">Due Settled (Debt Cleared)</div>
                                <div class="erp-kpi-value text-danger font-monospace">Rs {{ number_format($totalDueSettled, 2) }}</div>
                            </div>
                    ...
                    <div class="col-xl-3 col-sm-6">
                        <div class="erp-kpi-card">
                            <div>
                                <div class="erp-kpi-label">Cash Refunded (Paid)</div>
                    ...
                    <div class="px-4 py-3 bg-white border-bottom d-flex justify-content-between align-items-center" style="border-color: #dbeafe !important;">
                        <div class="d-flex align-items-center gap-2">
                            <span class="fw-bold text-dark" style="font-size: 0.90rem;">
                                <i class="fas fa-list-alt text-primary me-1"></i> Return Records List
                            </span>
                            ...
                            <table id="return-table" class="erp-table table align-middle mb-0">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-end" style="min-width: 110px;">Original Bill</th>
                                        <th class="text-end text-primary" style="min-width: 130px;">Return Goods Value</th>
                                        <th class="text-end text-danger" style="min-width: 110px;">Due Settled</th>
                                        <th class="text-end text-success" style="min-width: 110px;">Cash Refund</th>
                                ...
                                <tbody>
                                    @foreach ($returns as $return)
                                        @php
                                            $isPartialReturn = $return->sale && $return->total_returned < ($return->original_net_amount - 0.001);
                                            $isFullReturn = $return->sale && $return->total_returned >= ($return->original_net_amount - 0.001);
                                            
                                            $custDisplayName = $return->customer->customer_name ?? ($return->sale->walkin_name ?: 'Walking Customer');
                                            $isWalkin = empty($return->customer_id) || empty($return->customer);
                                            $initial = strtoupper(substr($custDisplayName, 0, 1));
                                        @endphp
                                        <tr>
                                            {{-- ID --}}
                                            ...
                                            <td>
                                                ...
                                                @if ($return->sale)
                                                    <div class="text-muted small mt-1" style="font-size: 0.68rem;">
                                                        <i class="fas fa-file-invoice text-primary me-1"></i>Orig: <strong class="text-dark">{{ $return->sale->invoice_no }}</strong>
                                                    </div>
                                                @endif
                                            </td>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center gap-2">
                                                    ...
                                                    <div>
                                                        <span class="fw-semibold text-dark">{{ $custDisplayName }}</span>
                                                        @if ($isWalkin)
                                                            <span class="badge bg-light text-muted border ms-1" style="font-size: 0.62rem;">Walk-in</span>
                                                        @endif
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            {{-- Return Goods Value --}}
                                            <td class="text-end font-monospace text-primary fw-bold" data-order="{{ (float)$return->net_amount }}">
                                                {{ number_format($return->net_amount, 2) }}
                                            </td>

                                            {{-- Due Settled (Debt Adjusted) --}}
                                            <td class="text-end font-monospace text-danger fw-bold" data-order="{{ (float)($return->due_adjusted ?? 0) }}">
                                                @if((float)$return->due_adjusted > 0)
                                                    -{{ number_format($return->due_adjusted, 2) }}
                                                @else
                                            ...
                                            {{-- Cash Refund Paid --}}
                                            <td class="text-end font-monospace fw-bold" style="color: #059669;" data-order="{{ (float)($return->paid ?? 0) }}">
                                                @if((float)$return->paid > 0)
                                                    {{ number_format($return->paid, 2) }}
                                                @else
                                                    <span class="text-muted">0.00</span>
                                                @endif
                                            </td>
                                            ...
                                            {{-- Action --}}
                                            <td class="text-center">
                                                <a href="{{ route('sale.return.view', $return->id) }}" class="btn-erp-table-action" title="View Return Details">
