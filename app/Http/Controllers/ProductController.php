...
class ProductController extends Controller
{
    ...
    public function product(Request $request)
    {
        ...
        if ($request->filled('item_type') && in_array($request->item_type, ['raw_material', 'finish_goods', 'both'])) {
    ...
    public function store_product(Request $request)
    {
        ...
        DB::transaction(function () use ($request, $userId, $nextCode, $imagePath, $mode, $height, $width, $piecesPerBox, $boxesQuantity,
            ...
            $product = Product::create([
                ...
                'item_type' => $request->input('item_type', 'raw_material') ?: 'raw_material',
                'is_part' => in_array($request->input('item_type', 'raw_material'), ['raw_material', 'both']) ? 1 : 0,
                'is_assembled' => in_array($request->input('item_type', 'raw_material'), ['finish_goods', 'both']) ? 1 : 0,
    ...
    */
    ...
    private function validateProductRequest(Request $request)
    {
        $rules = [
            ...
            'item_type' => 'nullable|in:raw_material,finish_goods,both',
